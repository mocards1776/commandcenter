...
export function TaskCard({ task }: { task: Task }) {
  ...
  // Shared timer toggle (no event — safe to call from context menu too)
  const toggleTimer = () => {
  ...
  const handleDelete = () => {
    if (confirm(`Delete "${task.title}"?`)) deleteMut.mutate();
  };

  // Completed task — slim deletable row with context menu
  if (task.status === "done") {
    return (
      <>
        <TaskContextMenu
